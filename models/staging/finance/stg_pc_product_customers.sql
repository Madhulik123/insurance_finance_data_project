...
data as (
    ...
    -- active customers have no churn date yet, so measure duration up to today
    date_diff(coalesce(date(churned_at), current_date()), date(started_at), day) as contract_duration_days
    from source_data
